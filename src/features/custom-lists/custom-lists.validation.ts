...
export const validateCreateList = [
  body("name")
    .trim()
    .notEmpty().withMessage(ERROR_MESSAGES.VALIDATION_LIST_NAME_REQUIRED)
    .isString().withMessage(ERROR_MESSAGES.VALIDATION_LIST_NAME_REQUIRED),
  handleInputErrors,
];
...
export const validateUpdateList = [
  param("listId")
    .isInt({ min: 1 }).withMessage(ERROR_MESSAGES.VALIDATION_LIST_ID_REQUIRED),
  body("name")
    .trim()
    .notEmpty().withMessage(ERROR_MESSAGES.VALIDATION_LIST_NAME_REQUIRED)
    .isString().withMessage(ERROR_MESSAGES.VALIDATION_LIST_NAME_REQUIRED),
