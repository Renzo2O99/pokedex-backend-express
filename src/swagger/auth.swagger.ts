...
export const authPaths = {
	"/auth/register": {
		post: {
			...
			responses: {
				...
				"400": {
					description: "Datos de entrada inválidos (ej. email no válido, contraseña corta).",
				},
	...
	"/auth/me": {
		get: {
			...
			tags: ["Autenticación"],
			security: [{ bearerAuth: [] }], // <-- Ruta protegida
			responses: {
				"200": {
	...
	"/auth/password": {
		put: {
			...
			description: "Actualiza la contraseña del usuario autenticado. Requiere la contraseña antigua y la nueva.",
			tags: ["Autenticación"],
			security: [{ bearerAuth: [] }], // <-- Ruta protegida
			requestBody: {
				required: true,
						...
						schema: {
							...
							properties: {
								...
								newPassword: {
									type: "string",
									format: "password",
									example: "nuevaContraseñaMasSegura456",
								},
							},
						},
					},
			...
			responses: {
				...
				"400": {
					description: "Datos de entrada inválidos (ej. contraseña nueva muy corta).",
				},
				"401": {
					description: "No autorizado (la 'oldPassword' es incorrecta).",
				},
			},
		},
	},
